...
#[derive(Debug, Clone)]
struct Node {
    state: String, // Replace with your actual game state type
    parent: Option<Node>,
    children: Vec<String>, // Replace with the actual game state type
    visits: f64,
    valid_actions: Vec<String>,
    action_probs: HashMap<String, f64>,
    action_counts: HashMap<String, f64>, // maybe we don't need this cause it's a tree?
    action_Qs: HashMap<String, f64>,
...
impl Node {
    ...
    fn uct_value(&self, action: String) -> f64 {
...
struct Tree {
    nodes: HashSet<String>, // replace with the actual type for the game states
    root: Node,
}
...
impl Tree {
    ...
    fn search(&self) -> (Node, String) {
        ...
        while true {
            let best_action = node.valid_actions
                .iter()
                .max_by(|a, b| {
            ...
            let next_state = node.do_move(best_action);
            ...
            if !node.children.contains(next_state) {
